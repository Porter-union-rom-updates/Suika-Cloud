...
module.exports = {
    ...
    onStart: async ({ api, event, args }) => {
        ...
        if (isNaN(amount) || amount <= 0) {
            return api.sendMessage('Please provide a valid amount to withdraw.', event.threadID);
        }
        ...
        if (bankDB[senderID].bankBalance < amount) {
            return api.sendMessage("You don't have enough balance in your bank to withdraw that amount.", event.threadID);
        }
        ...
        return api.sendMessage(`Successfully withdrew ${amount}. Your new balance is ${userDB[senderID].balance}.`, event.threadID);
    },
};
